...
const Home = () => {
  ...
  return (
    <div>
              ...
              streams.map((stream) => (
                <StyledTableRow key={stream.id}>
                  <StyledTableCell component="th" scope="row">
                    <div
                      className="tit"
                      onClick={() => history.push(`/StreamShow/${stream.id}`)}
                    >
                      {stream.title}
                    </div>

                    <div className="desc">{stream.description}</div>
                  </StyledTableCell>
                  ...
                  <StyledTableCell align="center">
                    ...
                    >
                      ...
                      <Button
                        id="2"
                        color="primary"
                        onClick={() => history.push(`/editStream/${stream.id}`)}
                      >
                        Edit
                      </Button>
                    </ButtonGroup>
      ...
      { user && <div className="btncreate">
        <Button
          onClick={() => history.push("/createStream")}
          color="primary"
          variant="contained"
          style={{ float: "left" }}
        >
